---
to: src/app/http/resources/<%= h.inflection.pluralize(name) %>/Update<%= h.inflection.singularize(Name) %>Resource.js
---
...
class Update<%= h.inflection.singularize(Name) %>Resource extends Retrieve<%= h.inflection.singularize(Name) %>Resource {
...
module.exports = {
    Update<%= h.inflection.singularize(Name) %>Resource
}